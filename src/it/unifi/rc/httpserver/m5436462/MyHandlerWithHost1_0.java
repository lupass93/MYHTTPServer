...
public class MyHandlerWithHost1_0 extends MyHTTPHandler1_0 implements HTTPHandler {
	 ...
	 * 
	 * @param host è {@link String} rappresenta l'host per il quale l'handler risponde alle richieste
	 * @param root è {@link File} rappresenta la directory a partire dalla effettua le dovute operazioni
	 */
	...
	public HTTPReply handle(HTTPRequest request){
		...
		if (!UtilityForMyProject.equalsIfNotNull(request.getParameters().get("Host"),this.host)) {  //Se l' Host della richiesta non corrisponde
			...
			return foundResponsable(request, parameters, myVersion);								//che provvederà a ritornare una risposta
		}
